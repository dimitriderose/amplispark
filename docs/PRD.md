...
Version 1.7 | March 15, 2026 (updated from v1.6 March 15)
...
| **Firebase Auth** | Google Sign-In + server-side verification | One-click Google authentication with persistent UID. Account dropdown with profile photo. Per-user brand isolation. Server-side Firebase Admin SDK verifies ID tokens on every API request via auth middleware. Brand ownership checks prevent cross-user access. |
| **Fernet Encryption** | OAuth token security | Notion OAuth access tokens encrypted at rest using Fernet symmetric encryption (cryptography library). `TOKEN_ENCRYPT_KEY` env var provides the encryption key. |
...
| Google Sign-In | **P1 — SHIPPED** | One-click Google authentication via Firebase `signInWithPopup` + `GoogleAuthProvider`. Account dropdown in NavBar shows profile photo and display name. Brands get an `owner_uid` field linking them to the authenticated Google user. On return, the app queries Firestore for all brands matching the UID and shows them on the dedicated Brands page (`/brands`). NavBar shows "My Brands" when signed in, "Get Started" when not. |
...
| UX polish | **P2 — SHIPPED** | Loading states with spinners on all async actions (including edit button). Empty states with helpful CTAs. Retry-on-failure patterns for API calls. Accessibility improvements (ARIA labels, keyboard navigation, focus management). useFetch hook and typed API layer for consistent data fetching. |
| Auth middleware | **P2 — SHIPPED** | Firebase Admin SDK server-side token verification on all API routes. `verify_brand_owner` dependency checks brand ownership before data access. Rejects unauthenticated or unauthorized requests with proper HTTP status codes. |
| Token encryption | **P2 — SHIPPED** | Fernet symmetric encryption (cryptography library) for stored OAuth tokens (Notion). `TOKEN_ENCRYPT_KEY` environment variable. Prevents plaintext token exposure in Firestore. |
| Security hardening | **P2 — SHIPPED** | 24-bug security audit: path traversal prevention on file operations, input validation on all user-supplied IDs, proper error handling to prevent information leakage, edge case handling for concurrent operations, encrypted token storage. |
...
16. **Security-first architecture** — Firebase token verification, Fernet encryption for OAuth tokens, path traversal prevention, server-side auth guards
...
*Last updated: March 15, 2026 (v1.6 — security hardening, multiplicative scoring, responsive design, modular architecture, AI media editor)*
