...
Version 1.8 | March 31, 2026 (updated from v1.7 March 15)
...
| **1. Onboarding (3-Step Wizard)** | A 3-step wizard replaces the old single-form onboarding. **Step 1:** Business name, description, website URL, and industry. **Step 2:** Tone, audience, brand colors, logo upload (entire step is skippable). **Step 3:** Platform selection — AI-recommended platforms or manual pick — then "Create My Brand." SessionStorage persistence means progress survives page refresh. Brand creation is deferred: all data is collected across steps and the brand is created on the final step. Plan generation has moved to the dashboard (EventsInput). **Onboarding guard:** navigating to `/onboard` when the user already has brands redirects to `/brands` — use the `?new=true` query parameter to explicitly start a new brand. "New Brand" buttons on the Dashboard and Brands page link to `/onboard?new=true`. | Clean wizard UI with step indicator (1/2/3). Each step validates independently. Back button returns to previous step. Step 2 has a "Skip" option for users who want defaults. Step 3 offers AI platform recommendations based on industry/audience or manual multi-select. "Create My Brand" triggers brand creation and navigates to the dashboard, where an 11-step guided tour begins automatically on first visit. Returning users who land on `/onboard` without `?new=true` are seamlessly redirected to their brand list. |
...
| **3. Strategy** | Strategy Agent takes the brand profile + user's goals (e.g., "launch spring menu") + optional business events this week (e.g., "new croissant Tuesday, farmer's market Saturday") and generates a multi-platform content calendar with variable briefs per day. The agent uses AI-researched posting frequency (Gemini + Google Search grounding, cached in Firestore) to determine optimal posts/week and best posting times per platform. A 7-day plan can produce 19-30+ briefs depending on the number of selected platforms and their researched cadences. Identifies 1-2 pillar ideas per week and plans platform-specific derivatives. Real business events become pillar anchors automatically. | Interactive calendar view showing each day's planned content theme and platform. Multiple cards stack per day column, sorted by suggested posting time. "What's happening this week?" text area above calendar -- disabled with contextual message while brand analysis is still running. Calendar generation shows a 6-step animated progress sequence (understanding brand, mapping events, building pillars, scheduling platform mix, crafting repurposing chains, finalizing calendar). A confirmation dialog is shown before clearing an active plan. Pillar posts are visually grouped. |
...
The Content Creator is called once per brief (multiple per day for multi-platform plans), producing one post per invocation. The Review Agent evaluates each post individually against the brand profile.
...
| **Cloud Firestore** | Brand profiles, content plans, post history, budget tracking | Real-time data sync. Generous free tier (1 GiB). Hierarchical data model maps cleanly to brand→plan→day→post. Budget tracker persists to Firestore, surviving Cloud Run cold starts and working correctly across auto-scaling instances. Paginated brand list and post list API endpoints prevent unbounded queries. |
...
| **Firebase Auth** | Google Sign-In + server-side verification | One-click Google authentication with persistent UID. Explicit `browserLocalPersistence` ensures sessions survive browser refresh. `ProtectedRoute` component wraps all authenticated routes — shows a loading spinner while Firebase restores auth state, preventing premature redirects to the landing page. All API calls include auth headers (9 previously missing fetch calls fixed). Account dropdown with profile photo. Per-user brand isolation. Server-side Firebase Admin SDK verifies ID tokens on every API request via auth middleware. Brand ownership checks prevent cross-user access. |
| **Fernet Encryption** | OAuth token security | Notion OAuth access tokens encrypted at rest using Fernet symmetric encryption (cryptography library). `TOKEN_ENCRYPT_KEY` env var is mandatory — the app refuses to start without it. HMAC-signed OAuth state prevents CSRF attacks on the Notion OAuth flow. |
...
User Browser (React 19) ←REST + SSE→ Cloud Run (FastAPI)
                                       ...
                                       ├── ProtectedRoute (loading spinner while Firebase restores auth state)
                                       ├── Global Exception Handler (structured error responses, no info leakage)
                                       ├── Structured JSON Logging (request_id, user_uid, duration)
                                       ├── Routers (modular — brands, plans, posts, generation, media, voice, integrations)
                                       ├── ADK Sequential Pipeline
                                       │   ├── Brand Analyst Agent (temp 0.15)
                                       │   ├── Strategy Agent
                                       │   ├── Content Creator Agent (interleaved output)
                                       │   │   ├── Carousel: 3-slide concurrent image gen (semaphore 7, safety validation)
                                       │   │   ├── Text-only regen mode (captions without re-generating images)
                                       │   │   └── Fallback: image-only retry on failure
                                       │   └── Review Agent (multiplicative scoring, auto-clean hashtags)
                                       ├── AI Media Editor (trend research + platform-optimized prompts)
                                       ├── Voice Coach (Gemini Live — BidiGenerateContent + calendar context)
                                       ├── Video Creator (Veo 3.1)
                                       ├── Firebase Google Auth (persistent UID + server-side verification)
                                       ├── Fernet Encryption (OAuth token storage)
...
brands/{brandId}/
  ...
  ├── selected_platforms: ["instagram", "linkedin", "x"]
  ├── platform_mode: "multi"           # "single" | "multi"
  └── content_plans/
      └── {planId}/
          ├── week_of: "2026-02-24"
          ...
          └── days/
              └── monday/
                  ...
                  └── status: "draft" | "approved" | "posted" | "deleted"
```
...
/onboard         → Brand setup wizard (URL, upload logo, describe business). Redirects to /brands if user already has brands — use ?new=true to explicitly add a new brand
...
/history         → Post History: searchable, filterable view of all posts across plans
  ├── Filters: status (draft/approved/posted), platform, content pillar
  ├── Search: full-text search across captions
  └── Pagination for large post libraries
...
| React dashboard | **P0 — Must Have** | Brand wizard + calendar view + generation view + post library. Fully responsive: mobile (<640px), tablet (641-1024px), and desktop layouts across all pages and components. Loading states, empty states, and retry-on-failure patterns throughout. |
...
| Description-first onboarding option | **P2 — Superseded** | Superseded by the 3-step onboarding wizard (P1 — SHIPPED). The wizard's Step 1 already makes business description the primary input with website URL as optional, addressing this requirement. |
...
| AI media editor | **P2 — SHIPPED** | Trend research + platform-optimized image prompts with text overlay support + adaptive style system that tailors visuals per platform. Style picker UX redesign with accessibility improvements. Platform-aware image and video prompt generation. |
| Responsive design | **P2 — SHIPPED** | Mobile-first responsive design across all pages and components. Dedicated tablet breakpoint (641-1024px). Touch-friendly controls, stacked layouts on mobile, side-by-side on desktop. |
| UX polish | **P2 — SHIPPED** | Loading states with spinners on all async actions (including edit button). Empty states with helpful CTAs. Retry-on-failure patterns for API calls. Accessibility improvements (ARIA labels, keyboard navigation, focus management). useFetch hook and typed API layer for consistent data fetching. Double-click protection on generation, video upload, and bulk operations. Error states displayed instead of silently swallowed. Batch stale post cleanup on list view. |
| Auth middleware | **P2 — SHIPPED** | Firebase Admin SDK server-side token verification on all API routes. `verify_brand_owner` dependency checks brand ownership before data access. Rejects unauthenticated or unauthorized requests with proper HTTP status codes. All 9 previously missing auth headers on frontend fetch calls have been fixed. |
| Token encryption | **P2 — SHIPPED** | Fernet symmetric encryption (cryptography library) for stored OAuth tokens (Notion). `TOKEN_ENCRYPT_KEY` environment variable is mandatory — app refuses to start without it. Prevents plaintext token exposure in Firestore. |
| Security hardening | **P2 — SHIPPED** | 24-bug security audit: path traversal prevention on file operations, input validation on all user-supplied IDs, proper error handling to prevent information leakage, edge case handling for concurrent operations, encrypted token storage. Tightened CORS with explicit allowed methods and headers (no wildcards). HMAC-signed OAuth state prevents CSRF on Notion flow. Mandatory `TOKEN_ENCRYPT_KEY` — app refuses to start without it. Error responses sanitized to prevent information leakage. |
| Modular architecture | **P2 — SHIPPED** | Backend refactored from monolithic `server.py` into modular routers (brands, plans, posts, generation, media, voice, integrations). Content creator split into focused modules (caption pipeline, image prompt builder, carousel builder, hashtag engine, quality gates). Shared utils, types, and constants across frontend and backend. Frontend gains `useFetch` hook and typed API layer. |
| Data safety | **P2 — SHIPPED** | Soft deletes: posts receive `status="deleted"` instead of being permanently removed, enabling future recovery. Budget tracker persists to Firestore, surviving Cloud Run cold starts and working correctly across auto-scaling instances. Pagination on brand list and post list API endpoints prevents unbounded queries. |
| Quality & observability | **P2 — SHIPPED** | 35 backend tests covering auth middleware, token encryption, budget tracking, brand sanitization, and post operations. Structured JSON logging with request context (request_id, user_uid, duration). Global exception handler for clean, consistent error responses. Metric logging for generation, auth failures, budget, and exports. |
...
4. **Instagram carousels** — 3-slide carousel posts with fully concurrent image generation (all slides at once), each following the brand's visual identity seed. Text-only regeneration mode lets users tweak captions without re-generating images
...
14. **Multiplicative scoring** — engagement scores combine platform-weighted dimensions with structural modifiers, not simple averages
15. **Full responsive design** — mobile, tablet, and desktop layouts with dedicated breakpoints, not just a desktop app
16. **Security-first architecture** — Firebase token verification, mandatory Fernet encryption for OAuth tokens, HMAC-signed OAuth state, tightened CORS, path traversal prevention, server-side auth guards
17. **Data safety** — soft deletes preserve posts for future recovery, budget tracker persists across Cloud Run cold starts and auto-scaling instances
18. **Quality & observability** — 35 backend tests, structured JSON logging with request context, global exception handler, metric logging for generation/auth/budget/exports
...
### Sprint 4 (Week 4): Export & Workflow — *SHIPPED*

All Sprint 4 features have been shipped. One-tap caption export, platform preview formatting, and content editing are live.

| Feature | Effort | Status |
|---|---|---|
| One-tap caption export | 3–4 hours | **SHIPPED** |
| Platform preview formatting | 1–2 days | **SHIPPED** |
| Content editing/regeneration | 1–2 days | Nice to Have |
| Competitor visibility toggle | 2–3 hours | Nice to Have |
...
### Sprint 6 (Week 7–8): Analytics & Intelligence — *partially SHIPPED*

Engagement prediction scoring (with multiplicative system) and Instagram grid visual consistency are shipped. Remaining features are post-hackathon.

| Feature | Effort | Status |
|---|---|---|
| Engagement prediction scoring | 3–5 days | **SHIPPED** (multiplicative scoring system) |
| Existing social media voice analysis | 3–5 days | Nice to Have |
| Platform meta intelligence | 2–3 days | Nice to Have |
| Instagram grid visual consistency | 2–3 days | **SHIPPED** (Brand Assets Service) |
...
*Last updated: March 31, 2026 (v1.8 — onboarding guard, auth reliability, carousel optimization, data safety, security enhancements, quality & observability, UX polish)*
